/*
This script was made to download the data of the Open Buildings 2.5D Temporal Dataset
...
/* The files will appear in the task tab that is on the right, the files can downloaded your google drive */
...
function exportBandForYear(year, bandCode) {
  ...
  Export.image.toDrive({
    ...
    region: geometryPolygon,  // The area to export
    fileFormat: 'GeoTIFF',  // File format
    maxPixels: 1e13  // Allow export of large images
  });
}

// Loop through the years and band codes and export each one
years.forEach(function(year) {
  bandCodes.forEach(function(bandCode) {
    exportBandForYear(year, bandCode);
  });
});
